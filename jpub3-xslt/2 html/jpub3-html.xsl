...
<!-- ============================================================= -->
  ...
  <xsl:template name="make-article">
	...
	<!-- id="{$this-article}-front" -->
    <header>
      <xsl:apply-templates select="front | front-stub"/>
    </header>

    <!-- body, id="{$this-article}-body" -->
    <xsl:for-each select="body">
      <section id="body" itemprop="articleBody">
        <xsl:apply-templates/>
      </section>
    </xsl:for-each>
    ...
    <xsl:if test="back | $loose-footnotes">
      <!-- $loose-footnotes is defined below as any footnotes outside
           front matter or fn-group, id="{$this-article}-back" -->
      <section id="back">
        <xsl:call-template name="make-back"/>
      </section>
    </xsl:if>

	<!-- floats, id="{$this-article}-floats" -->
    ...
    <!-- more metadata goes in the footer,id="{$this-article}-footer" -->
    <footer>
	<xsl:call-template name="footer-metadata"/>
	<p/>
     <!--  <xsl:call-template name="footer-branding"/>, transfered to /4 post/ -->
    </footer>
  ...
  <xsl:template match="front | front-stub">
    <div class="metadata">
        ...
        <!-- Article information -->
        <xsl:if test="article-meta | self::front-stub">
            <h4 class="generated">
  ...
  <xsl:template name="subsection-title"
    ...
    <xsl:if test="normalize-space($contents)">
	 ...
	 <xsl:if test="ancestor::body">
		<xsl:attribute name="id">
			<xsl:text>s</xsl:text>
				<xsl:number count="sec" level="multiple" format="1-1-1"/>
		</xsl:attribute>
		</xsl:if>
        <xsl:copy-of select="$contents"/>
      </xsl:element>
...
<!-- ============================================================= -->
  ...
  <xsl:template match="list" mode="list">
	...
	<xsl:template match="list-item">
		...
		<xsl:choose>
			<xsl:when test="./p">
				<xsl:apply-templates select="./p" mode="list"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates/>
			</xsl:otherwise>
  ...
  <xsl:template match="ref">
		<p class="ref">
			<xsl:attribute name="id">
				<xsl:value-of select="concat('B', position())"/>
			</xsl:attribute>
			<xsl:apply-templates/>
		</p>
  </xsl:template>
...
<!--   <xsl:template match="speech/speaker" mode="speech"/>
  ...
  <xsl:template name="backmatter-section">
    ...
    <section>
	<xsl:attribute name="id">
		<xsl:value-of select="lower-case($generated-title)"/>
	</xsl:attribute>
      <!-- <xsl:call-template name="named-anchor"/>-->
