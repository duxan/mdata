...
<!-- ============================================================= -->
  ...
  <xsl:template mode="mixed" match="italic">
	      <xsl:copy>
	              <xsl:apply-templates/>
	       </xsl:copy>
  </xsl:template>
  
   <xsl:template mode="mixed" match="sub">
	      <xsl:copy>
	              <xsl:apply-templates/>
	       </xsl:copy>
  </xsl:template>
  
   <xsl:template mode="mixed" match="sup">
	      <xsl:copy>
	              <xsl:apply-templates/>
	       </xsl:copy>
  </xsl:template>
  ...
  <xsl:template name="make-pages">
              ...
              <xsl:if test="volume"><xsl:value-of select="concat(', ',volume)"/></xsl:if>
              <xsl:if test="issue"><xsl:value-of select="concat('(',issue,')')"/></xsl:if>	
              <xsl:if test="fpage">
               	<xsl:if test="lpage"><xsl:value-of select="concat(', ',fpage,'-',lpage,'.')"/></xsl:if>
             		 <xsl:if test="not(lpage)"><xsl:value-of select="concat(', ',fpage,'.')"/></xsl:if>
              </xsl:if>
 
  </xsl:template>
  
  <xsl:template mode="mixed" match="name | fpage | lpage | volume | issue"/>

  <xsl:template mode="mixed" match="year">
	           <xsl:text>(</xsl:text>
                             <xsl:value-of select="."/>
                           <xsl:text>). </xsl:text>
  </xsl:template>
  
  <xsl:template mode="mixed" match="person-group">
	        <xsl:variable name="all-names" select="(person-group|name|collab|anonymous|etal|string-name)                 / (. | nlm:fetch-comment(.))"/><!-- $all-authors are all the elements that might be authors -->
		<xsl:variable name="placed-names" select="$all-names[not(preceding-sibling::* intersect             (current()/* except $all-names))]"/>
  ...
  <xsl:template name="journal-citation-content">
      ...
      <xsl:call-template name="format-date">
        <xsl:with-param name="date" select="$date"/>
	  		...
	  		<named-content content-type="mData prop name">
...
<!-- These templates are called by name and consolidate logic for
  ...
  <xsl:template name="format-names">
	...
	<named-content content-type="person-group">
        <xsl:call-template name="format-names">
          <xsl:with-param name="names"
...
<!-- 'author' mode formats structured name elements for the 
  ...
  <xsl:template match="name" mode="author">
    <xsl:document>
	<named-content content-type="mData type Person">
		<named-content content-type="mData prop familyName">
      <xsl:apply-templates select="surname" mode="format"/>
		</named-content>
      <xsl:text>, </xsl:text>
	  <named-content content-type="mData prop givenName">
    ...
    <xsl:if test="exists($date)">
      <xsl:choose>
        <xsl:when test="$date/self::date">
			<named-content content-type="mData prop datePublished">
          <xsl:call-template name="format-date">
            <xsl:with-param name="date" select="$date/*"/>
          </xsl:call-template>
			</named-content>
...
<!-- "format" mode: maps arbitrary citation element contents into
  ...
  <xsl:template mode="format" match="source">
    <named-content content-type="mData prop publisher">
      <xsl:apply-templates mode="format"/>
    </named-content>
  </xsl:template>
